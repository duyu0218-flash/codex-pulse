...
| Projects | Search, empty search, project details, project → task → project → list; live/recent/stale plan bars, current steps, update time and missing-plan count |
...
| Progress sync | Expand help in project/task/settings; copy instruction; assistant report automatically appears; new turn clears prior 100%; ended task retains final plan |
...
Progress regressions cover assistant-only report parsing, stripping extra fields, invalid/truncated/unsupported reports, user/tool/example rejection, fork exclusion, out-of-order updates, native plan events, turn reset, main/child aggregation, missing-plan counts, stale observation retention, completed-plan retention and v3 cache invalidation. Real-log before/after replay must preserve timing and token ledgers; keep those private artifacts outside the repository.
