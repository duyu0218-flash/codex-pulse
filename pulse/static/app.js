...
(() => {
  ...
  let data, busy = false, failed = false, search = '', filter = 'all', timer, reportBody = '', reportKey = '';
  ...
  const taskRows = (tasks) => tasks.length ? `<div class="cp-table-head"><span>任务 / 项目</span><span>状态与计划</span><span>累计耗时</span><span>Token</span></div>${tasks.map(t => `<div class="cp-task-row"><div>${link('task',t.id,t.title)}<div class="cp-task-project">${esc(t.projectName)}${t.child ? ' · 子代理' : ''}</div></div><div>${badge(t)}<div class="cp-step">${t.progress.known ? `${t.progress.completed} / ${t.progress.total} 步` : '进度未知'}</div></div><span class="cp-number">${time(t.duration)}</span><span class="cp-number" title="${num(t.usage.total_tokens)}">${token(t.usage.total_tokens)}</span></div>`).join('')}` : empty('此范围内没有任务记录');
  function metrics() {
    const m = data.metrics;
    return `<div class="cp-metrics"><div class="cp-metric"><div class="cp-metric-label">${failed ? '上次运行项目' : '当前运行项目'}</div><div class="cp-value cp-green">${m.runningProjects}<small>个</small></div><div class="cp-metric-foot">${failed ? '连接断开 · 上次观测' : '按最近日志观测'}</div></div><div class="cp-metric"><div class="cp-metric-label">当日运行过的项目</div><div class="cp-value">${m.projects}<small>个</small></div><div class="cp-metric-foot">${m.mainTasks} 个主任务 · ${m.childTasks} 个子代理</div></div><div class="cp-metric"><div class="cp-metric-label">回合累计耗时</div><div class="cp-value cp-duration">${time(m.duration)}</div><div class="cp-metric-foot">运行覆盖 ${time(m.wallTime)}</div></div><div class="cp-metric"><div class="cp-metric-label">当日 Token</div><div class="cp-value" title="${num(m.usage.total_tokens)}">${token(m.usage.total_tokens)}</div><div class="cp-metric-foot">输入 ${token(m.usage.input_tokens)} · 输出 ${token(m.usage.output_tokens)}</div></div></div>`;
  }
  function overview() {
    ...
    return `${metrics()}${attention.length ? `<div class="cp-attention">${attention.length} 个任务需要关注或确认状态 <button id="show-attention">查看任务</button></div>` : ''}<section class="cp-panel"><div class="cp-panel-top"><h2>${failed ? '上次运行项目' : '当前运行项目'}</h2><a href="#projects" class="cp-text-link">全部项目 →</a></div>${running.length ? `<div class="cp-running-grid">${running.map(p=>`<article class="cp-running-item"><div>${link('project',p.id,p.name)}<div class="cp-running-stage">${p.running} 个运行任务${p.waiting ? ` · ${p.waiting} 个等待` : ''}</div></div>${progress(p.progress,p.name)}</article>`).join('')}</div>` : empty('当前没有最近日志支持的运行任务')}</section><section class="cp-panel"><div class="cp-panel-top"><h2>任务活动 <span class="cp-muted">${data.tasks.length}</span></h2><div class="cp-tabs">${[['all','全部'],['running','运行中'],['attention','待关注']].map(([id,label])=>`<button data-filter="${id}" aria-pressed="${filter===id}">${label}</button>`).join('')}</div></div><div id="task-rows">${taskRows(filteredTasks())}</div></section>${timeline()}`;
  }
  ...
  function timeline() {
    ...
    const totals = Math.max(...active.map(p=>p.duration),1);
    return `<section class="cp-panel cp-pad"><h2>当日项目耗时</h2>${active.map(p=>`<div class="cp-stat-line">${link('project',p.id,p.name)}<span>${time(p.duration)}</span></div><div class="cp-bar"><span style="width:${p.duration/totals*100}%"></span></div>`).join('')}<p class="cp-muted cp-gap">含模型、工具与等待时间；并行回合累计可能超过自然日长度。</p></section>`;
  }
  function projectRows() {
    const projects = data.projects.filter(p=>p.name.toLowerCase().includes(search.toLowerCase()));
    return projects.length ? projects.map(p=>`<div class="cp-project-row"><div>${link('project',p.id,p.name)}<div class="cp-task-project">${p.tasks.length} 个任务 · ${p.running} 运行 · ${p.waiting} 等待</div></div>${progress(p.progress,p.name,!p.running&&!p.waiting&&!p.unknown)}<span class="cp-number">${time(p.duration)}</span><span class="cp-number">${token(p.usage.total_tokens)}</span></div>`).join('') : empty('没有匹配的项目');
  }
  function projects() {
  ...
  function project(id) {
    ...
    return `<a href="#projects" class="cp-text-link">← 返回项目</a><div class="cp-detail-title"><h2>${esc(p.name)}</h2><p class="cp-muted">${p.tasks.length} 个任务 · 累计 ${time(p.duration)} · ${num(p.usage.total_tokens)} Token</p></div><section class="cp-panel cp-pad cp-project-summary">${progress(p.progress,p.name,!p.running&&!p.waiting&&!p.unknown)}<p class="cp-muted cp-gap">项目进度汇总当前主任务的计划步骤；任一任务计划缺失时显示未知。</p></section><section class="cp-panel"><div class="cp-panel-top"><h2>项目任务</h2></div>${taskRows(data.tasks.filter(t=>p.tasks.includes(t.id)))}</section>`;
  }
  function task(id) {
    const t=data.tasks.find(x=>x.id===id);
    if (!t) return `<a href="#overview" class="cp-text-link">← 返回总览</a>${empty('此任务在当前日期没有记录。')}`;
    const plan=t.plan;
    return `${link('project',t.projectId,'← 返回项目','cp-text-link')}<div class="cp-detail-title"><h2>${esc(t.title)}</h2>${badge(t)}<p class="cp-muted cp-gap">${esc(t.model)} · ${t.child?'子代理':'主任务'} · 最后活动 ${new Date(t.last*1000).toLocaleString('zh-CN',{timeZone:data.timezone,hour12:false})}</p></div><div class="cp-details-grid"><div class="cp-detail-cell">当日累计耗时<strong>${time(t.duration)}</strong></div><div class="cp-detail-cell">当日 Token<strong>${num(t.usage.total_tokens)}</strong></div></div><section class="cp-panel cp-pad cp-gap"><h2>最近计划</h2>${progress(t.progress,t.title)}${plan ? plan.map((s,i)=>`<div class="cp-plan-row"><span class="cp-plan-mark ${s.status==='completed'?'done':s.status==='pending'?'':'current'}">${s.status==='completed'?'✓':i+1}</span><span>${esc(s.step)}</span><span class="cp-muted">${s.status==='completed'?'已完成':s.status==='pending'?'待开始':'进行中'}</span></div>`).join('') : '<p class="cp-muted cp-gap">该任务尚未记录结构化计划。</p>'}</section><section class="cp-panel cp-pad"><h2>当日回合</h2>${t.turns.map(r=>`<div class="cp-turn-row"><span>${r.start ? new Date(r.start*1000).toLocaleTimeString('zh-CN',{timeZone:data.timezone,hour12:false}) : '起点未知'}</span><span>${esc(names[r.status] || names.unknown)}</span><span>${time(r.seconds)}</span></div>`).join('') || empty('只有用量记录，缺少回合时间')}</section><details class="cp-panel cp-pad"><summary>数据归属</summary><p class="cp-muted cp-gap cp-wrap">工作目录：${esc(t.cwd)}<br>任务 ID：${esc(t.id)}</p></details>`;
  }
  function usage() {
    const u=data.metrics.usage;
    return `<div class="cp-two"><section class="cp-panel cp-pad"><h2>Token 构成</h2><div class="cp-value">${num(u.total_tokens)}</div><div class="cp-stack"><span style="width:${u.total_tokens?u.input_tokens/u.total_tokens*100:0}%;background:var(--cp-green)"></span><span style="flex:1;background:var(--cp-blue)"></span></div>${[['输入','input_tokens'],['其中缓存输入','cached_input_tokens'],['输出','output_tokens'],['其中推理输出','reasoning_output_tokens'],['缓存写入（来源字段）','cache_write_input_tokens']].map(([label,key])=>`<div class="cp-stat-line"><span>${label}</span><span>${num(u[key])}</span></div>`).join('')}<p class="cp-muted cp-gap">缓存输入已包含在输入中，推理输出已包含在输出中。</p></section><section class="cp-panel cp-pad"><h2>账户配额</h2><p class="cp-gap">暂未接入</p><p class="cp-muted cp-gap">${esc(data.quota.reason)} 可在 Codex 账户用量页查看官方额度。</p><p class="cp-muted cp-gap">金额估算未启用；本机 token 不能换算订阅剩余额度。</p></section></div><div class="cp-two"><section class="cp-panel cp-pad"><h2>按项目</h2>${data.projects.filter(p=>p.usage.total_tokens).map(p=>`<div class="cp-stat-line">${link('project',p.id,p.name)}<span>${token(p.usage.total_tokens)}</span></div><div class="cp-bar"><span style="width:${p.usage.total_tokens/Math.max(u.total_tokens,1)*100}%"></span></div>`).join('') || empty('当日暂无 token 记录')}</section><section class="cp-panel cp-pad"><h2>按模型</h2>${data.models.map(m=>`<div class="cp-stat-line"><span>${esc(m.model)}</span><span>${token(m.usage.total_tokens)}</span></div><div class="cp-bar"><span style="width:${m.usage.total_tokens/Math.max(u.total_tokens,1)*100}%"></span></div>`).join('') || empty('当日暂无模型记录')}</section></div><p class="cp-muted">旧格式响应 ${data.source.legacyResponses} 条；内部审批与记忆任务不计入项目用量。此统计不覆盖其他设备。</p>`;
  }
  function reportPage() {
  ...
  async function load(manual=false) {
    ...
    try {
      ...
      $('#scope').textContent=`此设备 · 最近 ${data.source.days} 天可查 · 账户额度未接入`;
